...
The reference architecture in this guide gives the ODS its own home: [Layer 3a](../blueprints/target-state.md), a purpose-built store for operational access, fed by source systems or by the EDP through the [serving patterns](../blueprints/coexistence-patterns.md). The [serving layer blueprint](../blueprints/serving-layer.md) covers the four shapes it can take and how each is fed. The EDP computes and historizes; the ODS or serving layer delivers current state at operational latency, under an operational SLA, in an operational failure domain. Caches, search indexes, and read-model projections are the same answer at different grains: purpose-built serving structures fed by systems of record or by the EDP, never the analytical tables themselves. Route the workload with the [decision tree](../decisions/decision-tree.md), and put a [contract](../patterns/data-contracts.md) on the flow between the two.
