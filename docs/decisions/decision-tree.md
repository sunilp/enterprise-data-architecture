...
**Converged platforms do not retire the tree.** Lakehouse vendors now ship managed operational databases beside the analytical engine, so both answers to this tree can land on one vendor platform. That changes where the workloads are deployed, not how they are routed. The question at every branch becomes "which failure domain and SLO regime does this workload live in," and the answer is reached the same way. A converged platform runs both kinds of workload; it does not make them the same workload. See [the convergence argument](../position/edp-vs-operational.md#the-convergence-argument).
